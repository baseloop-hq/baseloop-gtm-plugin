...
<example>
Context: User notices unexpected nulls in downstream fields.
user: "A bunch of rows have empty values in the email field even though enrichment ran successfully."
...
Before starting, load these Baseloop data-quality patterns into the audit: guessing extraction paths, non-text types for output fields, cascading field name changes, shortened/missing URLs, company name mismatches, domain mismatches, null propagation, stale field references, and silent type coercion.
...
2. For each table: `get_table_schema` — understand field structure.
...
- For each field that downstream fields depend on: what percentage of sampled rows have null values?
- For extraction fields: is `fullValue` populated but the extraction returning null? → Extraction path is wrong.
- For AI output fields: are nulls caused by upstream nulls feeding the prompt?
- For formula fields: are nulls caused by referencing a renamed/deleted field?

**Type coercion issues:**
- For extraction or AI output fields with type other than `text`: are values being silently coerced?
- Boolean fields receiving "Yes"/"No" instead of true/false → null.
- Number fields receiving non-numeric text → null.

**URL validation:**
- Call `list_actions` when you need to classify URL-dependent actions by current runtime `capabilities` such as `technographics.lookup`, `http.request`, or `ai.web_research`.
- For fields that feed URL-dependent actions:
...
- For rows where fields show `runConditionNotMet`: is this expected?
...
#### Critical
- **[table > field]**: [X/Y] sampled rows have null values despite upstream success
...
- **[table]**: [X/Y] rows have shortened URLs that will break enrichment
  - Affected field: [field feeding URL-dependent actions]
- **[table]**: Company name mismatch between [source A] and [source B] in [X] rows

#### Info
- **[table]**: [X/Y] rows blocked by autoRunCondition on [field] — verify this is intentional
...
- **Trace upstream** — when a value is null, always check what the upstream field produced.
