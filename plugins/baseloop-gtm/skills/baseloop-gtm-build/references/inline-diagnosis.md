...
When a field fails during Rung 1, fix it in place without abandoning the build. Two attempts max — escalate to `/baseloop-gtm-diagnose` if still broken.
...
- **After 2 fix attempts,** suggest the user run `/baseloop-gtm-diagnose` for a deeper investigation. Don't loop forever on the same field.
...
Stop inline-fixing and hand off to `/baseloop-gtm-diagnose` when:
